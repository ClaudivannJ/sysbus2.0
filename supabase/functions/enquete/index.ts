...
// Materializa a viagem de hoje respeitando o calendário (RF-16/17). Não cria
// ChamadaPonto (a chamada é o módulo do Monitor). Devolve o viagemId ou null.
async function garantirViagem(db: DB, destinoId: string): Promise<string | null> {
  ...
  if (!Array.isArray(destino.diasSemana) || !destino.diasSemana.includes(diaSemanaHojeISO())) return null;

  const inicioDia = dataHojeUTC();
  const { data: exc } = await db.from("ExcecaoCalendario").select("id")
  ...
  if (exc) return null; // feriado/facultativo/recesso

  if (!destino.enqueteAbre && !destino.enqueteFecha) return null; // rota sem enquete configurada
  ...
  if (error) return await viagemDeHoje(db, destinoId); // corrida: já criada
  return novoId;
}
...
Deno.serve(async (req) => {
  ...
  const viagemId = await garantirViagem(db, aluno.destinoId);
  if (!viagemId) return json({ viagem: null, fila: null, minhaReserva: null, autorizado: false });
