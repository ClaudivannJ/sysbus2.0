...
export default function Frota() {
  ...
  const { data: frota, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("Onibus").select("id,nome,placa,capacidade,motorista,ativo,destinoId,destino:Destino(nome)").order("nome");
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {(frota ?? []).map((o) => {
            ...
            return (
              <div key={o.id} className="flex flex-wrap items-center gap-3 rounded-xl bg-white p-4 ring-1 ring-slate-200">
