...
import { CalendarCheck, Check, X, Bus, CalendarX, CalendarOff, Landmark } from "lucide-react";
...
function SemViagem({ estado }: { estado: EstadoEnquete | null }) {
  const motivo = estado?.motivo ?? "SEM_VIAGEM";
  const prox = estado?.proximaData ? dataExtenso(estado.proximaData) : null;
  const cfg = {
    FORA_DE_OPERACAO: { Icon: CalendarOff, titulo: "Hoje não há transporte nesta rota", texto: "Este não é um dia de operação da sua rota." },
    FERIADO: { Icon: Landmark, titulo: "Sem transporte hoje (feriado / recesso)", texto: estado?.descricaoExcecao ? `Motivo: ${estado.descricaoExcecao}.` : "A secretaria marcou hoje como feriado ou recesso." },
    SEM_ENQUETE: { Icon: CalendarX, titulo: "Rota sem votação configurada", texto: "A secretaria ainda não definiu os horários de votação desta rota." },
    SEM_ROTA: { Icon: CalendarX, titulo: "Você ainda não tem uma rota", texto: "Defina sua cidade/rota no Perfil para participar da viagem." },
    SEM_VIAGEM: { Icon: CalendarX, titulo: "Não há viagem programada para hoje", texto: "Volte no próximo dia de operação da sua rota." },
  }[motivo];
  ...
  return (
    <div className="rounded-2xl bg-white p-6 text-center ring-1 ring-slate-200">
      ...
      <p className="mx-auto mt-1 max-w-xs text-sm text-slate-500">{texto}</p>
      {prox && (
        <p className="mt-3 inline-block rounded-lg bg-brand-50 px-3 py-2 text-sm text-brand-700 ring-1 ring-brand-200">
          Próxima viagem: <strong>{prox}</strong>{estado?.horarioSaida ? ` · saída ${estado.horarioSaida}` : ""}
        </p>
      )}
    </div>
  );
